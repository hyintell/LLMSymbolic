...
class SymbolicModuleInterface:
    ...
    def getValidCommands(self):
        ...
        return out
    def getValidCommands_with_obs(self, obs):
        out = []        
        for module in self.modules:            
            out.extend(module.getValidCommands_with_obs(obs))
...
class ModuleCalc(SymbolicModule):
    ...
    def runCommand(self, actionStr):
        ...
        if (opStr == "add"):
            ...
            result = int(arg1) - int(arg2)
            result = int(result)
            return "The result of subtracting " + str(arg2) + " from " + str(arg1) + " is " + str(result)
        elif (opStr == "mul"):
            result = int(arg1) * int(arg2)
            result = int(result)
...
class ModuleNavigation(SymbolicModule):
    ...
    def getNextStepToYCommand(self, endLocation):
        ...
        # If the startLocation is in the path, remove it
        if path[0] == startLocation:
            path = path[1:]
        
        # If the path length is 0, then the agent is already at the current location
        if (success == True) and (len(path) == 1):
            # return "You can directly choose actions to arrive at " + endLocation + "."
            return "You can directly choose action, 'move north', 'move south', 'move east' or 'move west' to arrive at " + endLocation + "."
            # return "To to arrive at " + endLocation + ", you can directly choose action from the following list: ['move north', 'move south', 'move east', 'move west']." + "."
            
            return "To to arrive at " + endLocation + ", you can choose action from the following list: ['move north', 'move south', 'move east', 'move west']."
            # return "To to arrive at " + endLocation + ", you can choose action to move."
        ...
        nextStep = path[0]
        return "The next location to go to is: " + nextStep + ".\n" + \
        "If you want to go to " + endLocation + " from " +  startLocation + ", you need go through " + ", ".join(path) + "."
...
class ModuleKnowledgeBaseTWC(SymbolicModule):
    ...
    #   Valid Commands
    #
    def getValidCommands_with_obs(self, observationStr):
        out = []        
        actionPrefix = "query " 

        for row in self.rows:
            # if row[0].strip() in observationStr or row[2].strip() in observationStr:
            pattern = r'\b' + re.escape(row[0].strip()) + r'\b'

            match_item = re.search(pattern, observationStr)
            pattern = r'\b' + re.escape(row[2].strip()) + r'\b'

            match_location = re.search(pattern, observationStr)
    ...
    def runCommand(self, actionStr):
        ...
        queryResultsStrs = [x[0] + ' is expected to be located at ' + x[2] for x in queryResults]
        queryResultsStrs = list(set(queryResultsStrs))
