...
def sanitizeInfo(infoIn, moduleInterface):
    ...
    # TODO make compatible with no symbolic modules
    if infoIn['taskDescription'] == \
        'Your task is to pick up objects, then place them in their usual locations in the environment.':
        # Add any valid actions from the module
        validActions.extend(moduleInterface.getValidCommands_with_obs(infoIn['observation']))
...
def T5Agent(args):
    ...
    agentName = "T5Agent"
    agentName, epName = lm.split("/")[-2:]
    # t5-base-finetuned-guidedby_LLM_zeroshot/ep_4_1024_32
    # Configure output dir to save the results
    # log_dir = os.path.join("evaluation_logs", gameName, 
    #                        setName, enabledModules, 
    #                        agentName, epName)
    log_dir = lm.replace("training_logs", "evaluation_logs")

    os.makedirs(log_dir, exist_ok=True)

    verboseFilenameOut = os.path.join(log_dir, f"{setName}_results.json")
    ...
    # History saver
    bufferedHistorySaver = BufferedHistorySaver(
        # filenameOutPrefix = args.historySavePrefix + "-game" + str(args.game_name) + "-lm" + str(lm).replace("/", "_") + "-" + str(args.set)
...
def main():
    ...
    if (args.train_or_eval == "train-gen"):
        ...
        T5Agent(args)
    elif (args.train_or_eval == "collect_data_by_LLM"):
        from play_game_by_LLM import CollectDatabyLLM
